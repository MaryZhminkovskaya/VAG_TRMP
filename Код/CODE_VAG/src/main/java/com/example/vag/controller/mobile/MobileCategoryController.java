...
public class MobileCategoryController {
    ...
    public ResponseEntity<?> deleteCategory(@PathVariable Long id) {
        try {
            ...
            // Проверяем, есть ли публикации в этой категории
            Long artworksCount = artworkService.countArtworksByCategoryId(id);
            if (artworksCount > 0) {
                Map<String, Object> response = new HashMap<>();
                response.put("success", false);
                response.put("message", "Cannot delete category with existing artworks");
                return ResponseEntity.badRequest().body(response);
            }
